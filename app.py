...
# ============================================================
# TEMPORARY — INITIALIZE REAL SEASON 1 CHAMPIONS VAULT RECORD
# ============================================================

@app.route(
    "/admin/initialize-season1-vault",
    methods=["GET", "POST"],
)
def initialize_season1_vault():
    """
    One-time Founder-only repair for the real historical Season 1.
    ...
    GET only displays a confirmation page.
    POST performs the database mutation.

    Remove this route after successful production initialization.
    """
    access = founder_required()
    if access:
        return access

    confirmation_key = "season1_vault_initialization_confirmed"

    if request.method == "GET":
        token = secrets.token_urlsafe(24)
        session[confirmation_key] = token
        ...
        <head>
            ...
            <title>Initialize Season 1 Champions Vault</title>
        </head>
        <body style="font-family:system-ui,sans-serif;max-width:720px;margin:40px auto;padding:24px;">
            <h1>Initialize Season 1 Champions Vault</h1>
            <p>This is a temporary Founder-only maintenance operation.</p>

            <ul>
                <li>Champion: <strong>Ntokozo</strong></li>
                <li>Runner-up: <strong>Inno</strong></li>
                <li>Final: <strong>3 - 1</strong></li>
            </ul>

            <p>
                This creates or restores the permanent Season 1 Champions Vault
                record. It does not alter future-season Vault automation.
            </p>
            ...
            <form method="post">
                ...
                >
                <button type="submit">
                    Confirm Season 1 Vault Initialization
                </button>
            </form>
        </body>
        </html>
    ...
    expected_token = session.pop(confirmation_key, None)
    ...
    if not expected_token or not supplied_token:
        return (
            "ABORTED: missing or expired confirmation token.",
            409,
        )

    if not secrets.compare_digest(expected_token, supplied_token):
        return (
            "ABORTED: invalid confirmation token.",
            403,
        )

    from models import HallOfChampion

    try:
        champion = (
            Player.query
            .filter(db.func.lower(Player.name) == "ntokozo")
            .first()
        )

        runner_up = (
            Player.query
            .filter(db.func.lower(Player.name) == "inno")
            .first()
        )

        if champion is None:
            return (
                "ABORTED: real player 'Ntokozo' was not found.",
                404,
            )

        if runner_up is None:
            return (
                "ABORTED: real player 'Inno' was not found.",
                404,
            )

        if champion.id == runner_up.id:
            return (
                "ABORTED: Ntokozo and Inno resolve to the same player.",
                409,
            )

        tournament = (
            Tournament.query
            .filter(Tournament.season_number == 1)
            .order_by(Tournament.id.desc())
            .first()
        )
        ...
        if tournament is None:
            tournament = (
                Tournament.query
                .filter(
                    db.func.lower(Tournament.name)
                    == "amination fc season 1"
                )
                .order_by(Tournament.id.desc())
                .first()
            )
        ...
        if tournament is None:
            ...
            if tournament.champion_id not in {
                None,
                champion.id,
            }:
                return (
                    "ABORTED: Season 1 already has a different champion.",
                    409,
                )

            if tournament.runner_up_id not in {
                None,
                runner_up.id,
            }:
                return (
                    "ABORTED: Season 1 already has a different runner-up.",
                    409,
                )

            tournament.status = TOURNAMENT_COMPLETED
            tournament.season_number = 1
        ...
        if existing_hall is not None:
            if (
                existing_hall.player_id == champion.id
                and existing_hall.final_score == "3 - 1"
            ):
                create_next_tournament_season_if_needed(tournament)
                db.session.commit()
                return (
                    f"ALREADY INITIALIZED: Season 1 | "
                    f"Champion={champion.name} | "
                    f"Runner-up={runner_up.name} | "
                    f"Final=3 - 1 | Hall ID={existing_hall.id}"
                )

            return (
                "ABORTED: Season 1 already has a conflicting Vault record.",
                409,
            )
        ...
        if hasattr(champion, "championship_stars"):
            champion.championship_stars = (
                champion.championship_stars or 0
            ) + 1

        hall_record = HallOfChampion(
            tournament_id=tournament.id,
            player_id=champion.id,
            team_name=getattr(champion, "team_name", None),
            season_number=1,
            tournament_name=tournament.name,
            final_score="3 - 1",
            matches_played=None,
        ...
        db.session.add(hall_record)
        create_next_tournament_season_if_needed(tournament)
        db.session.commit()

        return (
            f"INITIALIZED: Season 1 | "
            f"Champion={champion.name} | "
            f"Runner-up={runner_up.name} | "
            f"Final=3 - 1 | Hall ID={hall_record.id}"
        )
