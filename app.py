    ...
    if dialect == "sqlite":
        ...
        db.session.execute(text("""
            CREATE TABLE tournament_event (
                ...
                created_by VARCHAR(100),
                CONSTRAINT tournament_event_pkey
                    PRIMARY KEY (id),
                CONSTRAINT uq_tournament_event_sequence
                    UNIQUE (tournament_id, sequence_number),
                CONSTRAINT tournament_event_tournament_id_fkey
                    FOREIGN KEY (tournament_id)
                    REFERENCES tournament (id),
                CONSTRAINT tournament_event_match_id_fkey
                    FOREIGN KEY (match_id)
                    REFERENCES match (id),
                CONSTRAINT tournament_event_player_id_fkey
