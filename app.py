...
def migrate_analytics_schema():
    ...
    try:
        db.session.execute(
            text(
                ...
                CREATE TABLE analytics_event (
                    ...
                    CONSTRAINT fk_analytics_event_highlight
        ...
        db.session.execute(
            text(
                """
                CREATE INDEX ix_analytics_event_visitor_id
        ...
        db.session.execute(
            text(
                """
                CREATE INDEX ix_analytics_event_event_type
        ...
        db.session.execute(
            text(
                """
                CREATE INDEX ix_analytics_event_highlight_id
                ON analytics_event (highlight_id)
                """
            )
        )
        ...
        db.session.execute(
            text(
                """
                CREATE INDEX ix_analytics_event_occurred_at
        ...
        db.session.rollback()
        app.logger.exception(
            "Analytics production schema migration failed."
        )
        return {
            "success": False,
            "error": "Analytics schema migration failed."
        }, 500
